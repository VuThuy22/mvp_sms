...
//    long i = helper.addBank(new Bank(0L, "BIDV", "BIDV", 0.0));
    public static void handingSms(Context context, String message, Bank bank) {
        ...
        if (smsBank != null) {
            ...
            intent.setAction(Cts.ACTION_SMS);
            ...
            for (int i = 0; i < splitString.length; i++) {
                ...
                if (string.contains("ND:")) {
                    ...
                    for(int j=i+1;j<splitString.length;j++){
                        content = content+" "+splitString[j];
                    }
                }
                account=getAccout(content);
...
//Sacombank  11/07/2018 11:36 gui  3,484,000  VND  vao TK  020039754224  NAP_LUCKYBETS_01648681489 - So du kha dung:  19,915,373  VND
        ...
        try {
            ...
            for (int i = 0; i < splitString.length; i++) {
                ...
                if (string.contains("TK")) {
                    string ="";
                    for(int j=i+2;j<splitString.length;j++){
                        if(splitString[j].contains("-"))
                            break;
                        string = string+" "+splitString[j];
                    }
    ...
    public static String getAccout(String content){
        String utilContent="";
        String account="";
        utilContent=content.replace("_","").trim().replace(" ","");
        Log.e(TAG, "getSmsVietinBank: " + content.toUpperCase());
        if (utilContent.toUpperCase().contains("NAPLUCKYBETS")) {
            account = utilContent.toUpperCase().replace("NAPLUCKYBETS", "").trim();
        }
        return account;
    }
}
